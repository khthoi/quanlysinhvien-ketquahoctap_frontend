    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const res = await fetch(
                `http://localhost:3000/ket-qua/nhap-diem-excel?lop-hoc-phan=${lopHocPhanId}`,
                {
                    method: "POST",
                    headers: {
            ...
            if (res.ok) {
                showAlert("success", "Thành công", "Nhập điểm từ Excel thành công");
                handleClose();
                onSuccess();
            } else {
                const err = await res.json();
                showAlert("error", "Lỗi", err.message || "Nhập điểm thất bại");
            }
        } catch (err) {
